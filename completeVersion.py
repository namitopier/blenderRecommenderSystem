...
userProfileCache = []
numberOfOp = 0
globalLastOp = None # Last operation performed by the user. It is used to check if the user is performing the correct operation in the tutorial.
globalLastCorrectOp = None # Last correct operation performed by the user. It is used in case user performs an incorrect operation in the tutorial, so he knows what was the last correct one.
currentCorrectOpName = None # Name of the operation that is meant to be performed by the user as part of the tutorial.
tutorialMode = False
tutFileName = ""
ignoreLastOp = False
userFeedback = ""
recommenderInstance = None
globalFixedValues = [   "newVertices",
                        "newFaces",
                        "deletedVertices",
                        "deletedFaces",
                        "selectedVertices",
...
def getNewObjectInfo(objName):
    ...
    isMesh = activeObj.type == 'MESH'

    if isMesh:
        # Change to edit mode only if mesh
        bpy.ops.object.editmode_toggle()

    objDict = { "scale": list(activeObj.scale),
                "location": list(activeObj.location),
                "rotation": list(activeObj.rotation_euler),
                "vertices": {} if not isMesh else getAllVerticesOfObject(),
                "faces": {} if not isMesh else getAllFacesOfObject(),
                "isSmooth": True if (isMesh and any(face.use_smooth for face in activeObj.data.polygons)) else False}
...
def update_user_feedback(new_feedback, error = False):
    "Updates what is shown for the user in the UI"

    global globalLastCorrectOp

    if not error or globalLastCorrectOp is None:
        bpy.context.scene.user_feedback = new_feedback
    
    else:
        # Puts a tag in the feedback (!lastcorrect) to indicate the last correct operation.
        # In the function "split_text" this tag is used to split the text in the UI
        feedback = new_feedback + " !lastcorrect: " + globalLastCorrectOp
        bpy.context.scene.user_feedback = feedback
...
def split_text(text, width=40):
    """Splits the text into chunks of size width and checks for the tag !lastcorrect to split the text in the UI"""
    words = text.split(' ')
    lines = []
    current_line = ""
    ...
    for word in words:
        if word == "!lastcorrect:":
            ...
            lines.append(" ")
            current_line = "Last correct operation: "
...
def formatOperation2(operator, isSame):
    ...
    if isSame[:7] == "bpy.ops":
        ...
        if (mode and mode == "EDIT" and activeObj.type == "MESH"):
            ...
            if activeObj.name not in objsDict:
                # There are some cases where it automatically changes to edit mode from object mode
                
                newObjInfo = getNewObjectInfo(activeObj.name)
                newObjInfo["vertices"] = {}
                newObjInfo["faces"] = {}

                objsDict[activeObj.name] = newObjInfo
...
def getFilteredOp (translatedOp, additionalInfo = {"tolerance": 10}):
    ...
    global globalFixedValues
    fixedValues = copy.deepcopy(globalFixedValues)
...
class Tutorial:
    ...
    def __init__(self, tutorialName = None):
        ...
        self.tutorialSteps = [] # List of all the steps of the tutorial

        if tutorialName is not None and tutorialName != "":
            global currentCorrectOpName
            self.loadTutorialSteps(tutorialName)

            # Save the current correct operation name
            currentCorrectOpName = self.getNextStep()[0] if len(self.tutorialSteps) > 0 else None

            print("###################### DEBUG #########################")
            print("Current correct operation name: ", currentCorrectOpName)
            print("self.getNextStep(): ", self.getNextStep())
            print("len(self.tutorialSteps): ", len(self.tutorialSteps))
            print("#####################################################")
    ...
    def addTutorialStep(self, step):
        ...
        print("Included operation: ", filteredOp[0])
        # self.count = 0
        # print("\n============================= LIST OF OPS")
    ...
    def recursiveValidate(self, structure, structureCompare, structureType, tolerance, message = ""):
        ...
        if structureType == dict:
            for key in list(structure.keys()):
                value = structure[key]
                if key not in structureCompare:
                    # Means the operation is potentially different.
                    # Check if it was an additional fixed value
                    global globalFixedValues

                    if key not in globalFixedValues and key != "action":
                        # If it is not in globalFixedValues, means that it was a default value, thus it is wrong
                        message = "Expected value with name '{}' to be present in operation, but it is missing...".format(key)
                        print("\nRECEIVED: ", structureCompare)
                        print("\nEXPECTED: ", structure)
                        return [False, message]

                elif type(value) != float and type(value) != int:
                    response = self.recursiveValidate(value, structureCompare[key], type(value), tolerance, message = message)
                    ...
                    if not (value == valueCompare or (abs(value) >= valueCompare*(1-tolerance) and abs(value) <= valueCompare*(1+tolerance))):
                        message = "Expected value {} but got {}.".format(value, valueCompare)
                        return [False, message]
            ...
            if structureType != type(structureCompare) or len(structure) != len(structureCompare):
                # Means it is different
                return [False, message]

            for i, value in enumerate(structure):
                if type(value) != float and type(value) != int:
                    response = self.recursiveValidate(value, structureCompare[i], type(value), tolerance, message = message)
        ...
        elif structureType == str:
            # If it is a string, just have to check the value
            if structure == structureCompare:
                return [True, message]
            
            else:
                message = "Expected value {} but got {}.".format(structure, structureCompare)
                return [False, message]
    ...
    def validateFinalValues(self, tolerance, expectedMeshes, actualMesh, lastStep, objName):
        ...
        if not same:
            ...
            if expFacesLen != actFacesLen or expVertsLen != actVertsLen:
                ...
                if lastVertsLen != actVertsLen and lastVertsLen != 0:
                    ...
                    if (lastVertsLen > actVertsLen):
                        ...
                        if (missing == 1):
                            ...
                            update_user_feedback("ERROR FOUND! There is 1 vertex missing in this object. Be sure to add it at the correct location so the tutorial can continue!", error=True)
                        else:
                            ...
                            update_user_feedback("ERROR FOUND! There are " + str(missing) + " vertices missing in this object. Be sure to add them at the correct location so the tutorial can continue!", error=True)
                        ...
                        if (additional == 1):
                            ...
                            update_user_feedback("ERROR FOUND! There is 1 additional vertex in this object. Be sure to delete the correct one so the tutorial can continue!", error=True)
                        else:
                            ...
                            update_user_feedback("ERROR FOUND! There are " + str(additional) + " additional vertices in this object. Be sure to delete the corect ones so the tutorial can continue!", error=True)
                    ...
                    if (lastFacesLen > actFacesLen):
                        ...
                        if (missing == 1):
                            ...
                            update_user_feedback("ERROR FOUND! There is 1 face missing in this object. Be sure to add it at the correct location so the tutorial can continue!", error = True)
                        else:
                            ...
                            update_user_feedback("ERROR FOUND! There are "+ str(missing) +" faces missing in this object. Be sure to add them at the correct location so the tutorial can continue!", error = True)
                        ...
                        if (additional == 1):
                            ...
                            update_user_feedback("ERROR FOUND! There is 1 additional face in this object. Be sure to delete the correct one so the tutorial can continue!", error = True)
                        else:
                            ...
                            update_user_feedback("ERROR FOUND! There are "+ str(additional) +" additional faces in this object. Be sure to delete the corect ones so the tutorial can continue!", error = True)
                    ...
                    if (expVertsLen > actVertsLen):
                        ...
                        if (missing == 1):
                            ...
                            update_user_feedback("There is still 1 vertex missing in this object in order to conclude this step! Follow the tutorial to add it at the correct location!", error = True)
                        else:
                            ...
                            update_user_feedback("There are "+ str(missing) +" vertices missing in this object in order to conclude this step! Follow the tutorial to add them at the correct location!", error = True)
                        ...
                        if (additional == 1):
                            ...
                            update_user_feedback("There is still 1 additional vertex in this object in order to conclude this step! Follow the tutorial to delete it at the correct location!", error = True)
                        else:
                            ...
                            update_user_feedback("There are "+ str(additional) +" additional vertices in this object in order to conclude this step!. Follow the tutorial to delete them at the correct location!", error = True)
                    ...
                    if (expFacesLen > actFacesLen):
                        ...
                        if (missing == 1):
                            ...
                            update_user_feedback("There is still 1 face missing in this object in order to conclude this step! Follow the tutorial to add it at the correct location!", error = True)
                        else:
                            ...
                            update_user_feedback("There are "+ str(missing) +" faces missing in this object in order to conclude this step! Follow the tutorial to add them at the correct location!", error = True)
                        ...
                        if (additional == 1):
                            ...
                            update_user_feedback("There is still 1 additional face in this object in order to conclude this step! Follow the tutorial to delete it at the correct location!", error = True)
                        else:
                            ...
                            update_user_feedback("There are "+ str(additional) +" additional faces in this object in order to conclude this step!. Follow the tutorial to delete them at the correct location!", error = True)
                ...
                update_user_feedback("The current mesh topology is different than the expected for the conclusion of this step. Fix it in order to conclude this step! Follow the tutorial to shape it correctly!", error = True)

        return same, meshIndex


    def validateStep(self, step, currentStepOverride = None, stepOverrideIndex = 0):
        # Validates the step passed [operator.name, properties] with the current state of the tutorial
        # If currentStepOverride is passed, it will consider this step instead of the current state of the tutorial
        # stepOverrideIndex is the index of the step to be considered in the case of currentStepOverride
        # Returns a list with the result of the validation: ['correct'] if correct, ['wrong', filteredOp, self.tutorialSteps[self.state], message] if wrong and ['end'] if tutorial ended
        ...
        if currentStepOverride:
            ...
            stateIndexToConsider = stepOverrideIndex

        else:
            currentStep = self.tutorialSteps[self.state]
            stateIndexToConsider = self.state
        ...
        print("=========== PERFORMED VS EXPECTED = ", filteredOp[0], self.tutorialSteps[stateIndexToConsider][0])
        ...
        if ("editMode" not in currentStep[1] and editMode == "OBJECT") or ("editMode" in currentStep[1] and currentStep[1]["editMode"] == editMode):
            ...
            correct = False
            tolerance = self.tutorialSteps[stateIndexToConsider][-1]["tolerance"]/100
            meshIndex = -1
            ...
            if (editMode and activeObj and activeObj.name == currentStep[-2]):
                ...
                if "vertices" not in currentStep[1]:
                    ...
                    if filteredOp[0] == self.tutorialSteps[stateIndexToConsider][0]:
                        corrAndMessage = self.recursiveValidate(self.tutorialSteps[stateIndexToConsider], filteredOp, list, tolerance)
                        correct = corrAndMessage[0]
                        message = corrAndMessage[1]
                    
                    elif self.tutorialSteps[stateIndexToConsider][0].replace(" ", "") == "bpycontextspace_data" and "context" in self.tutorialSteps[stateIndexToConsider][1] and len(self.tutorialSteps[stateIndexToConsider][1].keys()) == 2:
                        # This is a special condition for the case that the operation is requiring user to click on a tab of properties (like material, object, modifier, etc)
                        # In this case, it might happen that the user is already in the correct tab, so need to validate the next step instead of the current one

                        print("##################################################################")
                        print("Especial case!")
                        print("Next step: ", self.tutorialSteps[stateIndexToConsider+1])
                        print("Performed step: ", filteredOp)

                        corrAndMessage = self.validateStep(step, currentStepOverride = self.tutorialSteps[stateIndexToConsider+1], stepOverrideIndex = stateIndexToConsider+1)
                        correct = True if corrAndMessage[0] == 'correct' else False
                        message = corrAndMessage[1] if not correct else ""
                        # corrAndMessage = self.recursiveValidate(self.tutorialSteps[stateIndexToConsider+1], filteredOp, list, tolerance)
                        # correct = corrAndMessage[0]
                        print("Corr and message: ", corrAndMessage)
                        # message = corrAndMessage[1]
                    ...
                    for i in range (stateIndexToConsider, stateIndexToConsider + numberOfMeshes if numberOfSteps >= stateIndexToConsider + numberOfMeshes else numberOfSteps):
                        # Considers the next 3 meshes states. In the case that current state + 5 overflows number of steps, considers until the end of steps
                    ...
                    correct, meshIndex = self.validateFinalValues(tolerance, meshes, actualMesh, self.tutorialSteps[stateIndexToConsider-1][1], objName)

            # Checking if the name of the operation is the same
            elif filteredOp[0] == self.tutorialSteps[stateIndexToConsider][0]:
                corrAndMessage = self.recursiveValidate(self.tutorialSteps[stateIndexToConsider], filteredOp, list, tolerance)
                correct = corrAndMessage[0]
                message = corrAndMessage[1]
            ...
            if correct:
                # If correct, update the state of the tutorial

                if(stateIndexToConsider == len(self.tutorialSteps) - 1):
                    return ['end']
                
                else:
                    # If found an equivalent mesh in the next operations, can skip some steps
                    if currentStepOverride:
                        self.state = stepOverrideIndex + 1

                    else:
                        if self.state == stateIndexToConsider: # If the state hasn't been updated yet by any of other recursive calls
                            self.state += 1 if meshIndex == -1 else meshIndex + 1 

                    # Update the current correct operation name
                    currentCorrectOpName = self.getNextStep()[0] if self.state < len(self.tutorialSteps) else currentCorrectOpName
                    return ['correct']

        return ['wrong', filteredOp, self.tutorialSteps[self.state], message] # List with wrong and correct operation
    ...
    def getProgress(self):
        ...
        if (self.state == len(self.tutorialSteps) + 1 ):
            ...
            return self.state/( len(self.tutorialSteps) + 1 )
...
class ModalOperator(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        elif event.type in {'LEFTMOUSE', 'RIGHTMOUSE','TAB', 'RET', 'INBETWEEN_MOUSEMOVE', 'DEL', 'MOUSEMOVE'}:

            isSame = isSameOperation(context.active_operator, event.mouse_x, event.mouse_y)

            if (type(isSame) != bool):
                # Means that it is a new operation
                ...
                if self.tutorialMode:
                    ...
                    if (result == ['correct']):
                        ...
                        globalLastCorrectOp = self.currOperation[0]

                        self.recommender.updateUserProfile(self.currOperation[0], True)
                        completionPercentage = "{:.1f} %".format(self.tut.getProgress() * 100)

                        print("============================ Correct operation!")
                        print("============================ Your progress: ", completionPercentage)
                        ...
                        update_user_feedback("Correct operation! Your current progress: " + completionPercentage + ". Next step:  Perform the following operation: " + str(stepDescription[0]) + " on object '" + str(stepDescription[-2]) + "'")
                        ...
                        if message == "":
                            ...
                            print("{}. Operation = {}".format(message, str(result[2][0])))

                        if bpy.context.scene.user_feedback[:7] == "Correct":
                            if message == "":
                                update_user_feedback("WRONG OPERATION! Expected operation: '" + str(result[2][0]) + "' but got: '" + str(result[1][0]) + "'", error=True)
                            else:
                                update_user_feedback("WRONG OPERATION! {}. Operation = {}".format(message, str(result[2][0])), error=True)
    ...
    def invoke(self, context, event):
        if context.object or context.object == None:
            ...
            global tutFileName
            global tutorialMode
            global recommenderInstance

            self.tut = Tutorial(tutorialName = tutFileName if tutorialMode else None)
            ...
            if tutorialMode:
                ...
                self.recommender = recommenderInstance
...
class recommenderSys:
    ...
    def updateUserProfile(self, operationName, correct, betaMult = 1.0):
        # Given an operation name and if the operation was correct, update user profile according
        # to alpha, beta and gamma.
        # betaMult is a multiplier for the beta value, allowing for dynamic adjustment of its influence. Used when manually registering difficulties.
        ...
        if operationName in self.allTerms:
            ...
            if correct:
                ...
                self.userProfile = np.add(np.multiply(self.userProfile, self.alpha), np.multiply(update, self.beta * betaMult))
    ...
    def makeRecommendation(self):
        # Returns a list of the top 2 recommendations (excluding the current one and the experiments ones: TUTExperiment1.txt and TUTExperiment2.txt)
        ...
        for tutName in self.termsDict['allTutorials'].keys():
            # Exclude the current tutorial (cannot recommend the same tutorial) and the experiments ones
            if tutName not in [tutFileName, "TUTExperiment1.txt", "TUTExperiment2.txt"]:
...
class ManualRegisterDifficulty(bpy.types.Operator):
    """Tooltip"""
    bl_idname = "object.manual_register_difficulty"
    bl_label = "Register User Difficulty"

    # This operator is used to manually register the user difficulties.
    # For example: Student asked help to professor in the current tutorial step. In this case, we are certain that
    # the user is having difficulties with the current operation, so we can register it manually, which assigns
    # double the weight for the recommender system.
    ...
    def execute(self, context):
        ...
        if recommenderInstance is not None:
            if currentCorrectOpName is not None:
                recommenderInstance.updateUserProfile(currentCorrectOpName, False, betaMult=2.0)

                oldFeedback = bpy.context.scene.user_feedback
                newFeedback = oldFeedback + " | User difficulty registered for operation: " + str(currentCorrectOpName)
                update_user_feedback(newFeedback, error=False)
                print("User difficulty registered for operation: ", currentCorrectOpName)
            else:
                self.report({'ERROR'}, "No correct operation name found to register difficulty.")
...
class LayoutDemoPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        # Manual register difficulty
        layout.label(text="Professor use ONLY:")
        row = layout.row()
        row.scale_y = 2.0
        row.operator("object.manual_register_difficulty")
...
def startLogger(context, tutMode = False, fileName = ""):
    ...
    if (tutMode):
        ...
        recommenderInstance = recommenderSys()
...
def stopLogger(reason = None):
    ...
    if reason == None:
        if tutorialMode:
            # Means the user stopped the logger before finishing the tutorial (could be an error, so recommend anyway)
            recommendations = recommenderInstance.makeRecommendation()

            if recommendations[0] == "" and recommendations[1] == "":
                update_user_feedback("Tutorial stopped early!. WARNING: The system could not gather enough information to make a recommendation nor identify your difficulties! Please, do more tutorials!")
            else:
                update_user_feedback("Tutorial stopped early! Difficulties identified in the following operations: " + str(recommenderInstance.getUserDifficulties()) + "    Tutorials recommended: " + str(recommenderInstance.makeRecommendation()))

        else:
            update_user_feedback("")

    else:
        # Means that the user has completed a tutorial, so add to the file statistics
        statistics_file_path = bpy.path.abspath('//user_statistics.txt')
        ...
        info_to_write = f"""
-------------------------------------------
Tutorial = {tutFileName}
Time taken = {time_taken:.2f}
Errors count = {errors_count}
"""

        if os.path.exists(statistics_file_path):
            # If file exists, open and append new info from completed tutorial
            with open(statistics_file_path, 'a', encoding='utf-8') as file:
                file.write(info_to_write)

        else:
            # If file doesn't exist, create and append new info from completed tutorial
...
def register():
    ...
    bpy.utils.register_class(ManualRegisterDifficulty)
...
def unregister():
    ...
    bpy.utils.unregister_class(ManualRegisterDifficulty)
