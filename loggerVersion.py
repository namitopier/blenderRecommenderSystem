            ...
            if (editMode and activeObj and activeObj.name == currentStep[-2]):
                ...
                if "vertices" not in currentStep[1]:
                    ...
                    if filteredOp[0] == self.tutorialSteps[self.state][0]:
                        corrAndMessage = self.recursiveValidate(self.tutorialSteps[self.state], filteredOp, list, tolerance)
                    ...
                    if (result == ['correct']):
                        ...
                        if bpy.context.scene.user_feedback[:7] == "Correct":
                            if message == "":
                                ...
                                update_user_feedback("WRONG OPERATION! {}. Operation = {}".format(message, str(result[2][0])))
...
class StartLogger(bpy.types.Operator):
    ...
    buttonType: bpy.props.StringProperty()
    ...
    def execute(self, context):
        startLogger(context, button = self.buttonType)
        return {'FINISHED'}
...
class LayoutDemoPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.scale_y = 3.0
        op = row.operator("object.log_actions", text = "Log exercise 1")
        op.buttonType = "exercise1"

        row = layout.row()
        row.scale_y = 3.0
        op = row.operator("object.log_actions", text = "Log exercise 2")
        op.buttonType = "exercise2"
...
def startLogger(context, tutMode = False, fileName = "", button = ""):
    global useLogger
    global tutorialMode
    global tutFileName
    global time_start
    ...
    if button == "exercise1":
        tutFileName = "Exercise 1"
    elif button == "exercise2":
        tutFileName = "Exercise 2"
...
def stopLogger(reason = None):
    ...
    for action in logCache:
        ...
        operations += format_list_as_string(action) + '\n'
